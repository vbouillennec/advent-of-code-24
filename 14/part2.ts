...
class Map {
	...
	executeRobots(nbOfExecutions: number) {
		for (let i = 0; i < nbOfExecutions; i++) {
			...
			if(nbQuad1 === nbQuad2 && nbQuad3 === nbQuad4) {
				...
				if(this.checkSymetry()) {
					this.print();
					console.log(`execution N°: ${i}`);
				}
			}
		}
	}
	...
	checkSymetry() {
		...
		for (const robot of this.robots) {
			...
			isSymetric = this.robots.find((opposedRobot) => {
				robot.position.x === (this.width - 1) - opposedRobot.position.x && 
				robot.position.y === opposedRobot.position.y
			}) !== undefined;
			// console.log('isSymetric ', isSymetric);
			if(isSymetric) {
				console.log('\nsymetric');
			}
			if(!isSymetric) {
