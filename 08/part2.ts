...
class Map {
	...
	generateAntinodes() {
		this.antennas.forEach((antenna1) => {
			if(this.antennas.some((antenna2) => this.isSameFrequencyAntenna(antenna1, antenna2))) {
				if(this.notInAntinodes(antenna1)) { 
					this.antinodes.push(antenna1);
				}
			}
			this.antennas.forEach((antenna2) => {
				if(this.isSameFrequencyAntenna(antenna1, antenna2)) {
					...
					let refPos  = antenna2;
					while(refPos) {
						refPos = this.setAntinode(refPos, vectorBetweenAntennas);
					}
				}
			});
		});
