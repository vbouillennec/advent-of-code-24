...
const bytesPos = input.split("\r\n").slice(0, NUMBER_OF_BYTES);
...
const canMove = (nextPos: Position): boolean => {
  console.log(`Checking position: ${nextPos.col},${nextPos.row}`);
  console.log(`Can move: ${!bytesPos.includes(`${nextPos.col},${nextPos.row}`)}`);
...
const lowestScore = findLowestCost(bytesPos);
